...
spawn(function()
    ...
    local ToggleSpawnCake = Tabs.Main:AddToggle("ToggleSpawnCake", {
        ...
        Description = " tắt là sẽ không spam katakuri v1", 
        Default = true })
...
while wait() do
    pcall(function()
        if _G.FastAttackWinhar then
            repeat wait(_G.DelayAttackPlayer)
               AttackNoCoolDown()
            until not _G.FastAttackWinhar
        end
    end)
end
end)
...
----------------------------------------------------------------------------------------------------------------------------------------------
InterfaceManager:SetFolder("Winharhub")
